...
ssh tommaso.fogliobonda@c3sfr1.di.unito.it "mkdir -p ~/swirlc-broadcast/rust"

# send the zip to c3sfr1.di.unito.it using scp
scp build/build.zip tommaso.fogliobonda@c3sfr1.di.unito.it:~/swirlc-broadcast/rust/build.zip

# unzip the file on the server
ssh tommaso.fogliobonda@c3sfr1.di.unito.it "cd ~/swirlc-broadcast/rust/ && unzip -o build.zip && rm build.zip"